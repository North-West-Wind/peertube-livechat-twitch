...
receiverClient.on("message", async message => {
	...
	if (swappable.client.isConnected && author) {
		await swappable.client.say(config.twitchChannel, `@${author.nickname} ${message.body}`);
	}
});
...
const swappable = new SwappableChatClient({ channels: [config.twitchChannel] });
